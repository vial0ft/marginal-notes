...
const Or = p => q => p(p)(q)
```
То заметим, что это в точности тоже самое что [`M` - комбинатор](#m-mockingbird), с той лишь разницей что мы добавляем дополнительный аргумент. 
...
Обратите внимание, что `n(k(f))` является **композицией**. И поэтому мы можем вспомнить про `B`-комбинатор и переписать выражение так:
```javascript
mult = n => k => f => B(n)(k)(f)
```
