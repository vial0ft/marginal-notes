...
Однако, это решение слабо параметризуется - если в [первом случае](#chaning) была возможность при необходимости передавать функции фильтрации или трансформации в качестве аргументов, то здесь это не очень простая задача.
...
Те простые функции и механизмы продемонстрированные в рамках этой заметки при работе с ленивыми вычислениями даже при "поддержке" *piping*, по мнению автора, по своим возможностям находятся на несколько ступеней ниже, чем [transdusers]ссылка), которые позволяют легко многократно использовать и переиспользовать логику по обработке данных.
